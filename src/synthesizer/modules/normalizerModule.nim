...
method synthesize*(module: NormalizerModule, x: float64, pin: int, moduleList: array[256, SynthModule], synthInfos: SynthInfos): float64 =
    if(module.inputs[0].moduleIndex < 0): return 0
    let moduleA = moduleList[module.inputs[0].moduleIndex]
    if(moduleA == nil): return 0.0
    let step = (2 * PI)/(synthInfos.oversample.float64 * synthInfos.waveDims.x.float64)
    var x1 = 0.0


    # for i in 0..<synthInfos.waveDims.x * synthInfos.oversample:
    #         let res = moduleA.synthesize((i.float64) * PI * 2 / synthInfos.waveDims.x.float64, module.inputs[0].pinIndex)
    ...
    if(module.update):
        if(moduleA == nil):
            ...
            module.min = 0
            while x1 < 2 * PI:
                let res = moduleA.synthesize(x1, module.inputs[0].pinIndex, moduleList, synthInfos)
                if(res > module.max): module.max = res
    ...
    return moduleA.synthesize(x, module.inputs[0].pinIndex, moduleList, synthInfos) * (1 / norm)
