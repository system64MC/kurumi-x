...
method synthesize*(module: WaveMirrorModule, x: float64, pin: int, moduleList: array[256, SynthModule]): float64 =
    ...
    if(moduleA == nil): 
        ...
        if(x < PI): return moduleA.synthesize(moduloFix(x2, 2 * PI), module.inputs[0].pinIndex, moduleList) else: return moduleA.synthesize(moduloFix(-(-2 * PI + x2), 2 * PI), module.inputs[0].pinIndex, moduleList)
