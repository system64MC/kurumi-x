...
method synthesize(module: FmodModule, x: float64, pin: int, moduleList: array[256, SynthModule]): float64 =
    ...
    let modulation = if(moduleA == nil): 0.0 else: moduleA.synthesize(x, module.inputs[0].pinIndex, moduleList)

    return moduleB.synthesize(x + modulation * 6, module.inputs[1].pinIndex, moduleList)
