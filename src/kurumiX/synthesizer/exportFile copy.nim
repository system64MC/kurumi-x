...
proc getSampleRate(synth: Synth): int =
...
when not defined(emscripten):
    ...
    proc saveWav*(data: string, bits: int = 16, sequence: bool = false): void {.EMSCRIPTEN_KEEPALIVE.} =
...
when not defined(emscripten):
    ...
    proc saveN163*(data: string, sequence: bool): void {.EMSCRIPTEN_KEEPALIVE.} =
        var synth = loadStateHistory(data)
        let tmpLen = synth.synthInfos.waveDims.x
        let tmpHei = synth.synthInfos.waveDims.y
...
when not defined(emscripten):
    proc saveFUW*(data: string): void {.gcsafe.} =
    ...
    proc saveFUW*(data: string): void {.EMSCRIPTEN_KEEPALIVE.} =
        var synth = loadStateHistory(data)
        let size: uint32 = 1 + 4 + 4 + 4 + (4 * synth.synthInfos.waveDims.x).uint32
        const HEADER_SIZE = 16 + 2 + 2 + 4 + 4 + 1 + 4 + 4 + 4
...
when not defined(emscripten):
    proc saveGenerator*(data: string) {.gcsafe.} =
    ...
    proc saveGenerator*(data: string) {.EMSCRIPTEN_KEEPALIVE.} =
...
when not defined(emscripten):
    proc saveAnalogGenerator*(data: string) {.gcsafe.} =
    ...
    proc saveAnalogGenerator*(data: string) {.EMSCRIPTEN_KEEPALIVE.} =
...
when not defined(emscripten):
    proc saveFMX*(data: string) {.gcsafe.} =
    ...
    proc saveFMX*(data: string) {.EMSCRIPTEN_KEEPALIVE.} =
